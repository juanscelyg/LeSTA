...
FeatureMap::FeatureMap(const HeightMap &map)
    : FeatureMap(map.getLength().x(), map.getLength().y(), map.getResolution()) {
...
FeatureMap::FeatureMap(double map_length_x, double map_length_y, double resolution) {
  ...
  for (const auto &feature_layer : feature_layers) {
    ...
    std::cout << "[LeSTA @FeatureExtraction] Added " << feature_layer << " layer to the height map"
              << std::endl;
  }
  ...
  for (const auto &normal_layer : normal_layers) {
    ...
    std::cout << "[LeSTA @FeatureExtraction] Added " << normal_layer << " layer to the height map"
              << std::endl;
  }
}

void FeatureMap::setNormalEstimationRadius(double radius) { normal_estimation_radius_ = radius; }

bool FeatureMap::initializeFrom(const HeightMap &map) {

  this->setFrameId(map.getFrameId());
  if (!map.hasHeightValues()) {
...
void FeatureMap::extractFeatures() {
  ...
  for (grid_map::GridMapIterator iterator(*this); !iterator.isPastEnd(); ++iterator) {
    ...
    if (this->isEmptyAt(CoreLayers::ELEVATION, *iterator))
      continue;
...
bool TerrainDescriptor::principleComponentAnalysisAt(const grid_map::HeightMap &map,
  ...
  for (grid_map::CircleIterator iterator(map, queried_cell_position, pca_region_radius_);
       !iterator.isPastEnd(); ++iterator) {
    Eigen::Vector3d point;
    if (!map.getPosition3(grid_map::HeightMap::CoreLayers::ELEVATION, *iterator, point))
      continue;
  ...
  if (descriptor_points_.size() < 4) {
    // debug
    // std::cout << "Not enough points to establish normal direction (nPoints = " << nPoints << ")" <<
    // std::endl;
    return false;
  }
...
bool TerrainDescriptor::doEigenDecomposition() {
  ...
  const auto covariance =
      squared_sum_points / descriptor_points_.size() - mean_points * mean_points.transpose();
...
float TerrainDescriptor::getSlope() const { return std::acos(std::abs(normal_vector_.z())) * 180 / M_PI; }

float TerrainDescriptor::getRoughness() const { return std::sqrt(eigen_values_(0)); }

float TerrainDescriptor::getCurvature() const { return std::abs(eigen_values_(0) / trace_); }

const double &TerrainDescriptor::getSmallestEigenValue() const { return eigen_values_(0); }

const Eigen::Vector3d &TerrainDescriptor::getNormalVector() const { return normal_vector_; }