...
public:
  ...
  template <typename T>
  static CloudPtr<T> applyTransform(const CloudPtr<T> &input, const geometry_msgs::TransformStamped &ts) {
  ...
  template <typename T>
  static CloudPtr<T> passThrough(const CloudPtr<T> &input, const std::string &field, double minVal,
  ...
  template <typename T>
  static CloudPtr<T> filterRange2D(const CloudPtr<T> &input, double minRange, double maxRange) {
    if (input->empty())
      return input;
    auto output = boost::make_shared<Cloud<T>>();
    output->header = input->header;
  ...
  template <typename T>
  static CloudPtr<T> filterRange3D(const CloudPtr<T> &input, double minRange, double maxRange) {
    if (input->empty())
      return input;
    auto output = boost::make_shared<Cloud<T>>();
    output->header = input->header;
  ...
  template <typename T>
  static CloudPtr<T> filterAngle2D(const CloudPtr<T> &input, double startDeg, double endDeg,
                                   bool invert = false) {
    if (input->empty())
      return input;
  ...
  template <typename T>
  static CloudPtr<T> downsampleVoxel(const CloudPtr<T> &input, double dx, double dy, double dz) {
    if (input->empty())
      return input;
    auto output = boost::make_shared<Cloud<T>>();
    pcl::VoxelGrid<T> vg;
  ...
  template <typename T> static CloudPtr<T> downsampleVoxel(const CloudPtr<T> &input, double leafSize) {
